...
def sendNotification(lst, tipster):
    sr_logo = 'soccer_rating_logo.ico' # Directory of the logo for Soccer-Rating.com

    toaster = ToastNotifier()

    # If there are more than one tips of the day, show them one by one with a delay of 5 seconds
    if len(lst) > 1:
        for count, item in enumerate(lst):
            toaster.show_toast("BetAlert -- {} new tips from {}! ({} / {})".format(len(lst), tipster, count+1, len(lst)), item, icon_path = sr_logo)
            while toaster.notification_active(): time.sleep(5) # Cooldown of 5 seconds between tips of same tipster
    else:
        toaster.show_toast("BetAlert -- New Tip from {}!".format(tipster), lst[0], icon_path = sr_logo)
